      ...
      async function fetchMovies() {
        try {
          ...
          const res = await fetch(
            `http://www.omdbapi.com/?i=tt3896198&apikey=59ebbdf8&s=${query}`
          );
  ...
  return (
    <>
      <NavBar>
        ...
        <NumResult movies={movies} setMovies={setMovies} />
      </NavBar>
      <Main>
        <Box>
          {isLoading && <Loader />}
...
function MoviesList({ movies, onSelected }) {
  return (
    <ul className="list list-movies">
      {movies?.map((movie) => (
        <Movies key={movie.imdbID} movie={movie} onSelected={onSelected} />
      ))}
    </ul>
  );
}
function Movies({ movie, onSelected }) {
  return (
...
function MovieDetails({ selectedId, onClosed, onAddWatched }) {
  ...
  return (
    <div className="details">
      {isLoading ? (
        ...
        <>
          <header>
            <button className="btn-back" onClick={onClosed}>
              ⬅️
            </button>
